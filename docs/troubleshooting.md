...
| No card ever appears | The source has no `feedback` block or it is `enabled: false`; or `TELEGRAM_BOT_TOKEN` is empty, which fills the queue without sending. `cli validate` prints `~> review in [...]` for every reviewed source. | Add the block, set a token, and wait one fetch interval — the fetch job seeds the chat. Send `/next` to force one immediately. |
| Cards stopped, queue is not empty | A card is still showing (only one unanswered at a time), or a claim's send failed and is waiting for the next fetch pass. `cli stats` shows `showing=` and `queued=` per chat. | Answer the card, or send `/next` in the review chat. If `showing=1` but no unanswered card is visible, clear the stuck claim with `UPDATE feedback_reviews SET state='queued', message_id=NULL WHERE state='pending'`. |
| An answered card still shows its buttons | The in-place edit was refused — a bot can only edit its own messages for 48 h. The label was written before any Telegram call, so nothing was lost, and the buttons are inert: a press on an answered card replies "already approved/rejected by …". | Nothing to fix; the next card is unaffected. |
